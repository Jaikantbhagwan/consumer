...
public class TestConsumerThread implements Runnable {
    ...
    public void run (){
        ...
        //List<ConsumerRecords<Long, String>> abc = new ArrayList<>;
        ...
        try {
            ...
            System.out.println("TopicAuthorizationException here 1:" + e);  
            final Consumer<Long, String> consumer1 = createConsumer1();
            System.out.println("Polling in TopicAuthorizationException block");
            try {
                //while (true) {
                    final ConsumerRecords<Long, String> consumerRecords = consumer1.poll(1000);
                    for(ConsumerRecord<Long, String> cr : consumerRecords) {
                        System.out.printf("Consumer Record:(%d, %s, %d, %d)\n", cr.key(), cr.value(), cr.partition(), cr.offset());
                    }
                    consumer.commitAsync();
                ...
                System.out.println("CommitFailedException: " + ce);
            } 
        } finally {
            consumer.close();
        }
    ...
    private Consumer<Long, String> createConsumer() {
        try {
            ...
            System.out.println("TopicAuthorizationException here 2: " + e);
        }
        return null;
    }
    
    private Consumer<Long, String> createConsumer1() {
        try {
            final Properties properties = new Properties();
            synchronized (TestConsumerThread.class) {
                properties.put(ConsumerConfig.CLIENT_ID_CONFIG, "KafkaExampleConsumer#" + id);
            ...
            //Get remaining properties from config file
            properties.load(new FileReader("src/main/resources/consumer.config"));
            properties.put("sasl.jaas.config", "org.apache.kafka.common.security.plain.PlainLoginModule required username=\"$ConnectionString\" password=\"Endpoint=sb://jaifirstnamespace.servicebus.windows.net/;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=JWasKBRjic0GsrUM6/UYS1aKy+miXtXvAlPjI0f5+y0=\";");
